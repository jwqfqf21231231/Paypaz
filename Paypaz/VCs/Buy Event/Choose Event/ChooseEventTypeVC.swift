    ...
    @objc func searchEventAsPerText(_ textField:UITextField)
    {
        ...
        if textField.text?.count != 0 {
            for eventData in self.eventData {
                let isMatchingContact : NSString = eventData.name! as NSString
                let range = isMatchingContact.lowercased.range(of: textField.text!, options: NSString.CompareOptions.caseInsensitive, range: nil, locale: nil)
                if range != nil {
